...
const Store = () => {
  ...
  const [SimilarAndPopularStore, setSimilarAndPopularStore] = useState([])
  ...
  const handleRegister = async (e) => {
    ...
    try {
      const response = await axios.post(
        `https://backend.qwiksavings.com/api/register`,
        {
          name: name1,
          email,
          password,
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const res = await axios.post(
        `https://backend.qwiksavings.com/api/login`,
        {
          email,
          password,
        }
    ...
    const fetchData = async () => {
      try {
        const res = await axios.get(
          `https://backend.qwiksavings.com/api/getStore/${sId}`
        )
        const similarAndPopularStore = await axios.get(
          `http://localhost:3000/api/similarAndPopularStore/${sId}`
        )
        // console.log(similarAndPopularStore)
        const coup = await axios.get(
          `https://backend.qwiksavings.com/api/coupons/${sId}`
        )
        setStr(res.data.store)

        const verifiedCoupons = coup.data.coupons.filter(
          (coupon) => coupon.isVerified
        )
        const aa = verifiedCoupons.sort((a, b) => {
          const dateA = new Date(a.created_at)
          const dateB = new Date(b.created_at)
          return dateB - dateA // Reverse the comparison to sort in descending order
        })
        // setSimilarAndPopularStore(similarAndPopularStore)
        setCoupons(verifiedCoupons)

        const response = await axios.get(
          `https://backend.qwiksavings.com/api/clouser`
        )

        const similarStores = response.data.data.filter(
          (item) => item.store_type === "similar" && item.store_id == sId
        )
        const popularStores = response.data.data.filter(
          (item) => item.store_type === "popular" && item.store_id == sId
        )
        const getStoreInfo = async (stores) => {
          return await Promise.all(
            stores.map(async (store) => {
              const res = await axios.get(
                `https://backend.qwiksavings.com/api/getStore/${store.sId}`
              )
    ...
    const fetchData = async () => {
      ...
      if (userId) {
        try {
          ...
          const response = await axios.get(
            `https://backend.qwiksavings.com/api/getDetails/${userId}`,
            config
          )
          const savedCouponsData = response.data.savedCoupons || []
          const likedCouponIds = savedCouponsData.map(
  ...
  const handleOpen = (product) => {
    if (open) {
      ...
      setOpen(!open)
      setWaiting(true)
      const correctedRefLink = product?.ref_link?.replace(
        /^(?:https?:\/\/)?(?:[^@\n]+@)?(?:www\.)?([^:/\n]+)/,
        "https://$1"
      )
  ...
  const handleReview = async (rate) => {
    ...
    try {
      await axios.put(
        `https://backend.qwiksavings.com/api/addRatings/${sId}`,
        data,
        {
          headers: {
  ...
  const handleLikeClick = async (index, cId) => {
    ...
    try {
      ...
      if (!likedItems.includes(cId)) {
        ...
        await axios.post(
          `https://backend.qwiksavings.com/api/saveCoupon/${cId}`,
          { userId },
          config
        )
      } else {
        ...
        await axios.delete(
          `https://backend.qwiksavings.com/api/unsaveCoupon/${cId}`,
          config
        )
      }
    } catch (error) {
  ...
  useEffect(() => {
    const counts = {
      exclusive: validCoupons?.filter((coupon) =>
        coupon.type.toLowerCase().includes("codes")
      ).length,
      rewards: validCoupons?.filter((coupon) =>
        coupon.type.toLowerCase().includes("reward")
      ).length,
      deals: validCoupons?.filter((coupon) =>
        coupon.type.toLowerCase().includes("deals")
      ).length,
      sales: validCoupons?.filter((coupon) =>
        coupon.type.toLowerCase().includes("sale")
      ).length,
    }
  ...
  return (
    <>
      ...
      <div className='mt-22 lg:my-32 flex flex-col lg:flex-row h-full max-w-[1280px] mx-auto'>
          ...
          <div className='flex flex-col gap-5 items-center justify-center'>
            <div className='flex gap-5 items-center'>
              ...
              <span className='font-bold whitespace-nowrap'>
                <span>{userRating}</span> Rating{" "}
              </span>
            </div>
            <div className='flex gap-5 items-center'>
              <span className='whitespace-nowrap text-lg font-bold'>
                Average Rating :{" "}
                {isNaN((str?.total_ratings / str?.ratings_count).toFixed(1))
                  ? 0
                  : (str?.total_ratings / str?.ratings_count).toFixed(1)}
              </span>
            </div>
          </div>
          <div className='lg:flex flex-col gap-5 hidden'>
            ...
            <div className='flex flex-col gap-2 shadow-boxshadow rounded-lg p-5 bg-white'>
              <div className='text-xl' style={{ fontWeight: 600 }}>
                Today{`'`}s Top {str?.name} Coupon Codes
              </div>
              {validCoupons &&
                validCoupons?.slice(0, 2).map((ele, index) => {
                  return (
                    <div
                      key={index}
                      className='w-full cursor-pointer list-disc bg-white rounded-lg flex gap-3'
                    >
                      <span className='text-lg'>- {ele.title}</span>
                    </div>
                  )
                })}
                ...
                {str?.faq && (
                  ...
                  >
                    <div className='flex gap-5 justify-between w-full'>
                      FAQS <FaQuestionCircle></FaQuestionCircle>
                    </div>
                  </Link>
                )}
                {str?.hint && (
                  ...
                  >
                    <div className='flex gap-5 justify-between w-full'>
                      How To Apply? <MdTipsAndUpdates></MdTipsAndUpdates>
                    </div>
                  </Link>
                )}
                {str?.moreAbout && (
                ...
                <div className='flex flex-wrap gap-2'>
                  {similarStoreNames.map((store, index) => (
                    <motion.div
                      ...
                      key={index}
                      className='text-sm p-1 duration-300  bg-gray-300 hover:bg-red-200 rounded-md cursor-pointer'
                      onClick={() => {
                        navigate(`/Stores/${store.name}`, {
                          state: { sId: store.id },
                        })
                      }}
                    >
        ...
        <div className='w-full lg:w-3/4 h-full flex flex-col border-l-2 lg:mx-5'>
          <div className='flex flex-col gap-3 justify-evenly p-5 pt-2'>
            <div className='lg:text-4xl text-2xl font-bold hidden lg:inline'>
              {str?.title}
            </div>

            <div className='text-md lg:text-sm font-semibold uppercase'>
              Best {validCoupons?.length} offers last validated on{" "}
              {console.log(validCoupons)}
              {validCoupons && validCoupons.length > 0
                ? validCoupons[0].due_date.split("T")[0]
                : "x"}
            </div>
            <Tabs value={activeTab}>
              <div className='flex flex-row items-center justify-between gap-3 lg:gap-5'>
                <TabsHeader
                  className='lg:w-full flex gap-2 lg:gap-4 bg-[#FAF9F5]'
                  style={{ zIndex: 10 }}
                >
                  <Tab
                    value='all'
                ...
                <div
                  className='flex items-center text-[#B33D53] hover:underline cursor-pointer whitespace-nowrap text-xs lg:text-base'
                  onClick={() => {
          ...
          <div className='flex flex-col gap-5 items-start lg:mx-5 mx-2'>
            {filteredCoupons &&
              filteredCoupons.map((ele, index) => {
                return (
                  <div
                    key={index}
                    className='group bg-white relative flex flex-col border border-gray-500 rounded-lg p-2 lg:p-5 w-full lg:w-full hover:shadow-lg duration-300'
                  >
                    <span
                      className={`p-2 hidden group-hover:inline-block duration-300 absolute right-1 top-1 rounded-lg bg-gray-300/80 ${
                        role && likedItems.includes(ele.coupon_id)
                          ? "text-red-500"
                          : "text-white"
                      }`}
                    ...
                    <div className='flex flex-col w-full gap-2'>
                      <div className='flex gap-1 lg:gap-0'>
                        <div className='w-[15%] h-auto flex flex-col items-center justify-center'>
                          <div className='border border-black flex flex-col items-center justify-center'>
                            <img
                              src={str?.logo_url}
                              alt='H'
                              className='max-h-[50px] h-auto w-[50px] lg:max-h-[75px]  lg:w-[75px] rounded-lg m-2'
                            />
                            <span className='bg-blue-100 text-center w-full capitalize text-xs lg:text-base'>
                              {ele.type}
                            </span>
                          </div>
                        </div>
                        <div className='flex flex-col w-[40%] lg:w-[55%] lg:mx-5 justify-between gap-2'>
                          <div className='flex flex-col lg:flex-row justify-between w-full mt-5'>
                            <div className='lg:font-bold text-[12px] lg:text-xl text-center lg:text-start'>
                              {ele.title}
                            </div>
                          </div>
                        </div>
                          ...
                          <div className='flex flex-col gap-5'>
                            <div className='flex whitespace-nowrap gap-3 lg:gap-5 lg:mr-[1rem] justify-between text-xs lg:text-base'>
                              <span className='flex justify-center items-center lg:gap-2 text-green-800'>
                                <GoVerified className='font-bold ' />
                                Verified
                              </span>
                            ...
                            {ele.type === "Codes" ? (
                              ...
                              <button
                                ...
                                onClick={handleOpen(ele, ele.type)}
                              >
                      ...
                      <div className='flex gap-2 items-center text-sm cursor-pointer justify-between lg:px-2 w-full'>
                        ...
                        >
                          {" "}
                          See Details{" "}
                          <IoAddOutline className='cursor-pointer'></IoAddOutline>
                        </span>
                        <span className='flex gap-4 lg:gap-20 items-center justify-between text-sm cursor-pointer lg:mr-1 h-10'>
                          <span className='whitespace-nowrap text-sx lg:text-base'>
                            41 % Success
                          </span>
                          <span className='flex items-center gap-2 lg:gap-7 w-full lg:text-xl'>
                            {" "}
                            <FaRegThumbsUp
                              className='hover:scale-125 duration-200 lg:h-5 lg:w-5'
                              onClick={() => handelLike(ele)}
                            ></FaRegThumbsUp>
                            <FaRegThumbsDown
                              className='hover:scale-125 duration-200 lg:h-5 lg:w-5'
              ...
              {expiredCoupons?.map((ele, index) => (
                ...
                >
                  <div className='flex flex-col w-full gap-2'>
                      ...
                      <div className='w-[15%] h-auto flex flex-col items-center justify-center '>
                        <div className='border border-black flex flex-col items-center justify-center text-xs lg:text-base'>
                          <img
                            src={str?.logo_url}
                            alt='H'
                            className='max-h-[50px] h-auto w-[50px] lg:max-h-[75px] lg:w-[75px] rounded-lg m-2 contrast-50 grayscale'
                          />
                          <span className='bg-blue-100 text-center w-full capitalize'>
                            {ele.type}
                          </span>
                        </div>
                      </div>
                        ...
                        <div className='flex flex-col gap-5'>
                          <div className='flex whitespace-nowrap gap-3 lg:gap-5 lg:mr-[1rem] justify-between text-xs lg:text-base'>
                            <span className='flex justify-center items-center lg:gap-2 text-red-800'>
                              <MdOutlineSentimentDissatisfied className='text-xl' />
                              Expired
                            </span>
                            <span className='flex justify-center items-center lg:gap-2'>
                              <CiUser></CiUser>
                          ...
                          {ele.type === "Codes" ? (
                            <button
                              className='button has-code1 !grayscale'
                              onClick={() => {
                                // console.log("asd")
                            ...
                            <button
                              className='!grayscale whitespace-nowrap bg-[#B33D53] px-4 py-2 text-white rounded-md'
                              onClick={(e) => {
                                // console.log("asd")
                    ...
                    <div
                      className='flex gap-1 items-center cursor-pointer text-xs lg:text-base lg:mx-2 lg:mt-2'
                      onClick={() => toggleDetails(index + validCoupons.length)}
                    >
                      See Details{" "}
                      <IoAddOutline className='cursor-pointer'></IoAddOutline>
                    </div>
              ...
              {validCoupons &&
                validCoupons?.slice(0, 2).map((ele, index) => {
                  return (
                    <ul
                      key={index}
                      className='w-full cursor-pointer list-disc bg-white p-2 rounded-lg flex gap-3 ml-1'
                    >
                      <li className='font-semibold text-[12px] pl-2'>
                        {ele.title}
                      </li>
                    </ul>
                  )
          ...
          {str?.faq && (
            <div
              className='w-full lg:mx-5 p-10 bg-white my-2 shadow-boxshadow rounded-lg'
              id='faqs'
            >
                ...
                {str?.faq?.map((ele, index) => {
                  return (
                    <div
                      ...
                      initial='hidden'
                      key={index}
                      className='flex flex-col gap-2 lg:text-justify'
                    >
          ...
          {str?.hint && (
            <div
              ...
              id='hints_tips'
            >
              <div className='font-semibold lg:text-4xl text-xl my-3'>
                How to apply?
              </div>
              <div className='moreaboutcompany flex flex-col gap-2'>
                {str?.hint?.includes("\n") ? (
      ...
      <Dialog
        open={open}
        handler={handleOpen(e)}
        size='lg'
        className='relative text-black p-5'
      >
        <IoMdClose
          className='text-black h-6 w-6 absolute right-5 top-5 cursor-pointer'
          onClick={() => handleClose()}
        />
        <div className='flex flex-col items-center'>
          <div className='flex flex-col gap-3 lg:gap-5 items-center'>
            ...
            <div className='flex flex-col gap-5 justify-center items-center flex-wrap'>
              ...
              <div className='text-sm lg:text-3xl font-semibold text-black whitespace-nowrap'>
                {selectedProduct.title}
              </div>
            </div>
            <div className='text-lg'>
              Ends {formatDate(selectedProduct.due_date)}
            </div>
            {selectedProduct.type == "Deals" ? null : (
              <div className='flex items-center lg:min-w-[20rem] w-fit max-w-full justify-center border border-black rounded-full text-xl pl-10 p-2 bg-red-50/40'>
                <span className='copy-text w-[60%] text-center'>
            ...
            {!waiting ? (
              <div className='text-xs lg:text-2xl text-green-800 w-full flex items-center justify-center gap-5'>
                ...
                <a
                  ...
                  rel='noopener noreferrer'
                  className='whitespace-nowrap duration-300 underline text-[#800000]
                        cursor-pointer flex items-center gap-2'
                >
                  {selectedProduct.name && selectedProduct?.name.toUpperCase()}{" "}
                  {str?.name} Product
                  <TbExternalLink />
                </a>
              </div>
            ) : (
              <div className='text:sm lg:text-2xl text-[#800000]'>
                Wait for 2 Second...
              </div>
            )}
      ...
      <Dialog open={openlogin} handler={handleOpenlogin} className='relative '>
        <span
          className='absolute top-5 right-5 text-3xl cursor-pointer z-10'
          onClick={() => closeboth()}
        >
          <IoMdClose></IoMdClose>
        </span>
        ...
        <Card
          ...
          shadow={false}
        >
          <div className='text-2xl text-black font-semibold mb-2 mt-10 lg:mt-20'>
            Login
          </div>
          <div className='mt-4 mx-auto font-normal text-black my-2'>
            <span>
              ...
              New customer?{" "}
              <span
                className='underline font-medium text-red-500 transition-colors hover:text-red-800 cursor-pointer'
          ...
          <div className='bg-white p-10 rounded-xl border flex flex-col gap-5 my-10'>
            <form className='w-56 max-w-screen-lg lg:w-96 mx-auto'>
              ...
              <Typography color='gray' className='mt-2 mx-auto font-normal'>
                <Link
                  to='https://backend.qwiksavings.com/api/forgot-password'
                  className=' underline font-medium transition-colors hover:text-orange-700 cursor-pointer'
                >
                  Forgot your password?
                </Link>
              </Typography>
              <Button
                className='mt-6 bg-[#800000]'
                type='submit'
                onClick={handleLogin}
                fullWidth
              >
              ...
              <span className='text-sm text-black font-extralight'>
                By continuing, I agree to RetailMeNot’s
      ...
      >
        ...
        <Card
          ...
          shadow={false}
        >
          <div className='text-4xl text-black font-semibold mb-2 mt-10 lg:mt-20'>
            Join Now
          </div>
          <div className='mt-4 mx-auto font-normal text-black my-2 cursor-pointer'>
            <span>
              ...
              <span
                className='underline font-medium text-red-500 transition-colors hover:text-red-800'
                onClick={() => {
                  handleOpenregister()
                  openlogin()
                }}
            ...
            <form className='w-56 max-w-screen-lg lg:w-96 mx-auto'>
              <div className='mb-4 flex flex-col gap-6  items-center justify-center'>
                ...
                <span className='text-sm text-gray-500'>
                  Password must be 8 characters or more, Don&apos;t use any part
                  of your email, Don&apos;t use a common password
                </span>
              </div>
              <Button
                className='mt-6 bg-[#800000] rounded-full cursor-pointer'
                type='submit'
