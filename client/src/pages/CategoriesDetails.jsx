  ...
  useEffect(() => {
    const fetchData = async () => {
      const response = await axios.get(
        `https://backend.qwiksavings.com/api/coupon/${category}`
      )

      const res = await axios.get(`https://backend.qwiksavings.com/api/clouser`)

      const popularStores = res.data.data.filter(
        (item) => item.store_type === "popular" && item.category_id == catId
      )
        ...
        <div className='flex flex-col items-start justify-center flex-wrap gap-5'>
          <ul className='flex items-center ml-1'>
            <li className='inline-flex items-center'>
              <Link to='/' className='text-gray-900 hover:text-[#B33D53]'>
                <svg
                  className='w-5 h-auto fill-current mx-2 text-gray-900'
              ...
              <span className='h-auto text-gray-400 font-medium'>/</span>
            </li>
            <li className='inline-flex items-center'>
              <Link
                to='/all-categories'
                className='text-gray-900 hover:text-[#B33D53] whitespace-nowrap'
              >
                Categories
              </Link>
              <span className='h-auto text-gray-400 font-medium'>/</span>
          ...
          <div className='flex flex-col lg:flex-row'>
              ...
              <div className='lg:flex flex-col gap-5 text-sm items-center hidden'>
                  ...
                  <div className='flex flex-wrap gap-2 text-sm'>
                    {Categories.length !== 0 &&
                      Categories.map(
                        (c, i) =>
                          c.name === category && (
                            <p className='text-start'>{c.About}</p>
                          )
                      )}
                  </div>
                </div>
                <div className='min-w-full shadow-boxshadow rounded-lg p-5 bg-white'>
                  ...
                  <div className='flex flex-wrap gap-2'>
                    {Categories.slice(0, 20).map(
                      (ele, index) =>
                        ele.todays_top === 1 && (
                          <div
                            key={index}
                            className='cursor-pointer text-sm p-1 duration-300 bg-gray-300 hover:bg-red-200 rounded-md'
                            onClick={() => {
                  ...
                  <div className='flex flex-wrap gap-2'>
                    {firstLatter.map((ele, index) => (
                      <div
                        ...
                        onClick={() =>
                          navigate("/all-stores", { state: { letter: ele } })
                        }
                      >
                        {ele}
                      </div>
            ...
            <div className='w-full lg:w-3/4 h-full flex flex-col border-l-2 lg:mx-5 lg:-mt-12'>
              <div className='p-4 lg:pt-0 text-xl lg:text-3xl font-bold'>
                {category}
              </div>
              <div className='flex flex-col gap-2 lg:gap-5 items-start lg:mx-5 mx-2 mb-10'>
                {couponDetails.slice(0, eventsToShow).map((ele, index) => {
                  return (
                    <div
                      key={index}
                      className='group bg-white relative flex flex-col border border-gray-500 rounded-lg p-2 lg:p-5 w-[340px] lg:w-full hover:shadow-lg duration-300'
                    >
                      <span
                        ...
                        <div className='flex gap-1 items-center text-sm cursor-pointer justify-between lg:pt-5 lg:px-2 w-full'>
                          ...
                          <span className='flex gap-4 lg:gap-20 items-center justify-between text-sm cursor-pointer lg:mr-1 h-10'>
                            ...
                            <span className='flex items-center gap-2 lg:gap-7 w-full lg:text-xl'>
                              {" "}
                              <FaRegThumbsUp className='hover:scale-125 duration-200 lg:h-5 lg:w-5'></FaRegThumbsUp>
                              <FaRegThumbsDown className='hover:scale-125 duration-200 lg:h-5 lg:w-5'></FaRegThumbsDown>
                            </span>
                          </span>
                        </div>
