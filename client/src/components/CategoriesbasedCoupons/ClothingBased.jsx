...
const ClothingBased = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get("https://backend.qwiksavings.com/api/storeDisplay/Clothing");
    ...
    <>
      ...
      <div className='clothing mb-5'>
        <div className='flex flex-col justify-center items-center lg:flex-row lg:justify-between my-10 h-[2rem]'>
          ...
          <div
            className='hover:underline h-7 duration-300 underline cursor-pointer'
            onClick={() => {
              navigate("/categoriesdetails", {
                state: { category: "Clothing", category_icon: Clothing },
              })
            }}
          >
        ...
        <div className='lg:grid grid-cols-1 lg:grid-cols-4 justify-items-stretch gap-4 overflow-auto flex flex-wrap justify-center md:justify-start scroll-snap-type-x mandatory scrollbar-hide'>
          {clothes
            ...
            .map((item, index) => (
              <div
                key={index}
                className="group flex flex-col gap-3 items-center justify-start relative max-w-sm h-auto border rounded-lg overflow-hidden shadow-lg duration-300 my-4 pb-10 bg-white overflow-y-hidden item flex-shrink-0 scroll-snap-align-start"
                style={{ width: "290px" }}
              >
                <img
                  src={item.thumbnail}
                  ...
                  onClick={() =>
                    navigate(`/Stores/${item.name}`, {
                      state: { sId: item.id },
                    })
                  }
                />
                <span
                  className={`p-2 hidden group-hover:inline-block duration-300 absolute right-1 top-1 rounded-lg bg-gray-300/80 ${role && likedItems.includes(item.coupon_id)
                      ? "text-red-500"
                      : "text-white"
                    }`}
                ...
                <div className='ml-24 flex w-[60%] justify-end items-center text-gray-700 '>
                  {item.isVerified && (
                    <span className='text-black bg-blue-200 px-1 rounded-md text-[12px] uppercase'>
                      Verified
                    </span>
                  )}
                </div>
                <div
                  className='mx-4 h-[48px] p-2 text-start cursor-pointer'
                ...
                <div className='flex flex-col items-center justify-between'>
                  <button
                    className='button has-code'
                    onClick={() => handleOpen(item)}
                  >
                    <span className='is-code'>74{item.coupon_code}</span>
                    <span className='is-code-text uppercase'>
                      <em>Get {item.type}</em>
                    </span>
                  </button>
                </div>
          ...
          <div className='bg-white p-10 rounded-xl border flex flex-col gap-5 my-10'>
            <form className='w-56 max-w-screen-lg lg:w-96 mx-auto'>
              ...
              <span className='text-sm text-black font-extralight'>
                ...
                <span className='underline font-bold cursor-pointer'>
                  {" "}
                  <Link to='/terms-of-services'>Terms & use</Link>
                </span>
              </span>
            </form>
          </div>
