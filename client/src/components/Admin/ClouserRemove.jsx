...
export function ClouserRemove({ storeId, open, handleOpen }) {
                ...
                const storeDetails = await Promise.all(
                    storesData.map(async store => {
                        if (store.store_type === displayOption) {
                            const storeResponse = await axios.get(`${import.meta.env.VITE_LOCAL_SERVER}/api/getStore/${store.sId}`);
    ...
    const handleRemoveStore = async (store_id) => {
        if (store_id) {
            try {
                await axios.delete(`${import.meta.env.VITE_LOCAL_SERVER}/api/admin/removeClouser/${store_id}`, {
                    headers: {
