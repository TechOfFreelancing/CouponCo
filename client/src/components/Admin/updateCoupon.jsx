  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const events =
          eventName.length !== 0 &&
          eventName.data.coupons.map((e) => e.event_name)
        setSelectedEvents(events)
        const presentInHomePage = result.data.data.some(
          (item) => item.coupon_id === cId
        )
