...
export function ClouserCRm({ categoryId, open, handleOpen }) {
            ...
            try {
                ...
                const storeDetails = await Promise.all(
                    storesData.map(async store => {
                        if (store.category_id === categoryId) {
    ...
    const handleRemoveStore = async (store_id) => {
        if (store_id) {
            try {
                await axios.delete(
                    `https://backend.qwiksavings.com/api/admin/removePopularForCategory/${store_id}`,
                    {
                        headers: {
