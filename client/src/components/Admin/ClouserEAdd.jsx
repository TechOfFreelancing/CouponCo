...
export function ClouserE({ eventId, open, handleOpen }) {
        ...
        const fetchData = async () => {
            try {
                ...
                const associatedStoreIds = storesData
                    .filter(store => store.event_id === eventId)
                    .map(store => store.sId);
    ...
    const handleStoreClick = async (clickedStoreId) => {
        try {
            const res = await axios.post(`https://backend.qwiksavings.com/api/admin/addPopularForEvent/${clickedStoreId}`, {
                event_id: eventId,
            },
                {
                    headers: {
