...
export function Clouser({ storeId, open, handleOpen }) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`${import.meta.env.VITE_LOCAL_SERVER}/api/getAllStore`);
    ...
    const handleStoreClick = async (clickedStoreId) => {
        if (displayOption && clickedStoreId) {
            try {
                const res = await axios.post(`${import.meta.env.VITE_LOCAL_SERVER}/api/admin/addClouser/${storeId}`, {
