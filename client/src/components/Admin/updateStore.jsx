...
function UpdateStores() {
  ...
  const handleFormOpen = () => {
    setOpenDisplayForm(!openDisplayForm)
  }

  const handleClouserOpen = () => {
    setOpneClouserForm(!openClouserForm)
  }

  const handleDeleteClouserOpen = () => {
    setOpneDeleteClouser(!openDeleteClouser)
  }
  ...
  const handleRemoveFrom = async (showcaseItem) => {
    try {
      ...
      await axios.delete(
        `https://backend.qwiksavings.com/api/storeDisplay/${sId}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
  ...
  const handleRemoveFromFestivalOffer = async () => {
    try {
      await axios.delete(
        `https://backend.qwiksavings.com/api/admin/deleteStoreFromFest/${sId}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
    ...
    const fetchData = async () => {
      try {
        ...
        const offerRes = await axios.get(
          `https://backend.qwiksavings.com/api/festStoreDisplay`
        )

        const presentInOffer = offerRes.data.data.some(
          (item) => item.storeId === sId
        )
        setIsPresentInOffer(presentInOffer)

        const presentInHomePage = result.data.data.some(
          (item) => item.store_id === sId
        )
        isPresentInHomePage.current = presentInHomePage

        if (presentInHomePage) {
          const showcaseItems = result.data.data.filter(
            (item) => item.store_id === sId
          )
        ...
        toast.error(
          error.response ? error.response.statusText : "Failed to fetch data"
        )
    ...
    <>
      ...
      <div className='max-w-[1280px] mx-auto p-4 bg-white rounded-lg'>
        ...
        <form onSubmit={formik.handleSubmit}>
          <div className='mb-4'>
            <label className='block mb-1 font-medium' htmlFor='storeFile'>
              Upload image:
            </label>
            <input
              id='storeFile'
          ...
          <div className='flex gap-4'>
            ...
            <button
              type='button'
              onClick={() =>
                navigate("/Admin/addCoupons", { state: { sId: sId } })
              }
