...
function UpdateStores() {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            try {
                ...
                const updateUrl = `${import.meta.env.VITE_LOCAL_SERVER}/api/admin/updateStore/${sId}`;
                ...
                if (validFaqs.length > 0) {
                    ...
                    const faqUrl = `${import.meta.env.VITE_LOCAL_SERVER}/api/admin/addFaq/${sId}`;
    ...
    const handleRemoveFromFestivalOffer = async () => {
        try {
            await axios.delete(`${import.meta.env.VITE_LOCAL_SERVER}/api/admin/deleteStoreFromFest/${sId}`, {
                headers: {
                    "Authorization": `Bearer ${localStorage.getItem('token')}`
                },
            });
    ...
    const handleAddToFestivalOffer = async () => {
        try {
            await axios.post(
                `${import.meta.env.VITE_LOCAL_SERVER}/api/admin/addStoreToFest/${sId}`,
                {},
                {
                    headers: {
                        Authorization: `Bearer ${localStorage.getItem('token')}`,
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`${import.meta.env.VITE_LOCAL_SERVER}/api/getStore/${sId}`);

                const result = await axios.get(`${import.meta.env.VITE_LOCAL_SERVER}/api/storeDisplay`);

                const offerRes = await axios.get(`${import.meta.env.VITE_LOCAL_SERVER}/api/festStoreDisplay`);
