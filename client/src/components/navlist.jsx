    ...
    const fetchStores = async () => {
      try {
        ...
        if (response && response.data && response.data.stores) {
          ...
          const sortedUniqueStores = Object.values(uniqueStores).sort((a, b) =>
    ...
    const fetchData = async () => {
      try {
        ...
        if (response && response.data && response.data.categories) {
          ...
          const uniqueCategories = response.data.categories.reduce(
            (acc, current) => {
              const firstLetter = current.name.charAt(0).toUpperCase()
              if (!acc[firstLetter]) {
                acc[firstLetter] = current
    ...
    <ul className='py-5  mb-4 flex flex-col gap-2 lg:mb-0 lg:flex-row lg:items-center lg:gap-6 text-black'>
      ...
      >
        <Link
          ...
          className='flex items-center hover:-translate-y-1 duration-300 hover:text-red-500 hover:lg:border-red-500 hover:lg:border-b-2 whitespace-nowrap'
        >
          Stores
        </Link>
        <div
          className={`lg:block absolute top-0 lg:-left-[19.8rem] transition ${
            isStoresDropdownOpen
              ? "opacity-100 visible translate-y-5"
              : "opacity-0 invisible translate-y-5"
          } duration-500 ease-in-out transform z-50 w-[100vw] max-w-[1280px] mx-auto`}
          onClick={closeDropdowns}
        >
      ...
      >
        ...
        <div
          className={`lg:block absolute top-0  lg:-left-[25.3rem] transition ${
              ...
              <div
                ...
                onClick={() =>
                  navigate("/categoriesdetails", {
                    state: { category: ele.name, category_icon: ele.logo_url },
                  })
                }
                className='cursor-pointer hover:-translate-y-1 duration-300 hover:text-red-500 hover:underline'
              >
      ...
      <span className='p-1 font-normal font-[Poppins]'>
        <Link
          to='/our-codes'
          className='flex items-center hover:-translate-y-1 duration-300 hover:text-red-500 hover:lg:border-b-2 hover:lg:border-red-500 whitespace-nowrap'
        >
          Our Codes
        </Link>
      </span>
      ...
      >
        ...
        <div
          className={`lg:block absolute top-0 lg:-left-[40.3rem] transition ${
              ...
              <div
                ...
                onClick={() =>
                  navigate("/eventdetails", {
                    state: { eId: ele.id, event: ele.event_name },
                  })
                }
                className='cursor-pointer hover:-translate-y-1 duration-300 hover:text-red-500 hover:underline'
              >
      ...
      <span className='p-1 font-normal font-[Poppins]'>
        <Link
          to='/blogs'
          className='flex items-center hover:-translate-y-1 duration-300 hover:text-red-500 hover:lg:border-red-500 hover:lg:border-b-2'
        >
          Blog
        </Link>
      </span>
