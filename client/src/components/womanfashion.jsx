            ...
            try {
                ...
                if (response.data && response.data.data) {
                    ...
                    const store = await Promise.all(fetchedImagesWithId.map(async store => {
                        try {
                            const storeDetails = await axios.get(`process.env.URL/api/getStore/${store.storeId}`);
                    ...
                    const coupon = await Promise.all(fetchedImagesWithId.map(async coupon => {
                        try {
                            const couponDetails = await axios.get(`process.env.URL/api/coupons/${coupon.storeId}/${coupon.cId}`);
