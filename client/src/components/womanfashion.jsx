...
const Womanfashion = () => {
    ...
    useEffect(() => {
        const fetchImages = async () => {
            try {
                const response = await axios.get(`http://43.205.126.26:3000//api/storeDisplay`);
                if (response.data && response.data.data) {
                    ...
                    const store = await Promise.all(fetchedImagesWithId.map(async store => {
                        try {
                            const storeDetails = await axios.get(`http://43.205.126.26:3000//api/getStore/${store.storeId}`);
                    ...
                    const coupon = await Promise.all(fetchedImagesWithId.map(async coupon => {
                        try {
                            const couponDetails = await axios.get(`http://43.205.126.26:3000//api/coupons/${coupon.storeId}/${coupon.cId}`);
