...
exports.addToTodaysTop = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    const [storeResult] = await db.query("SELECT * FROM store WHERE id = ?", [
      storeId,
    ])
    ...
    const [isExist] = await db.query(
      "SELECT * FROM store_display WHERE coupon_id = ?",
      [couponId]
    )
    if (isExist.length > 0) {
      return next(
        new ErrorHandler(`Coupon with ID ${couponId} is already displayed`, 400)
      )
    }
    ...
    const [couponInfoResult] = await db.query(
      "SELECT category, due_date FROM coupons WHERE coupon_id = ?",
      [couponId]
    )
    ...
    if (new Date(dueDate) < new Date()) {
      return next(
        new ErrorHandler(
          `Coupon with ID ${couponId} is expired and cannot be added to today's top offer`,
          400
        )
      )
    }
    ...
    if (validCurrentCount >= maxLimit) {
      return next(
        new ErrorHandler(
          `Cannot add more than ${maxLimit} valid coupons to today's top offer`,
          400
        )
      )
    }
    ...
    const sql =
      "INSERT INTO store_display (store_id, coupon_id, show_in_top, thumbnail) VALUES (?, ?, ?, ?)"
    ...
    console.error(err)
    return next(
      new ErrorHandler("Unable to add this to today's top offer", 500)
    )
  }
})
...
exports.getSingleStore = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: `Store with ID ${storeId} fetched successfully`, store })
  } catch (err) {
    console.error(err)
...
exports.similarAndPopularStore = catchAsyncErrors(async (req, res) => {
  const { id } = req.params
  console.log(id)
  try {
    // Fetch the store details by ID
    const data = await db.query(
      "SELECT * FROM Coupons.store_ids join store on store_ids.store_id = store.id where store_id = ?;",
      [id]
    )
    const store = data[0]
    res
      .status(200)
      .json({ message: `Store with ID ${id} fetched successfully`, store })
  } catch (err) {
    console.error(err)
    res.status(400).json({
      message: `Unable to fetch ID ${id}.`,
    })
    // return next(new ErrorHandler("Unable to fetch store", 500))
  }
})
...
exports.updateStore = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    res.status(200).json({
      message: `Store with ID ${storeId} updated successfully`,
...
exports.addStoreFAQs = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    const faqArray = Object.entries(faq).map(([question, answer]) => ({
      answer,
      question,
    }))
    ...
    res.status(200).json({
      message: "Store FAQs updated successfully",
...
exports.addStoreRating = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Rating added successfully", store: updatedStore })
  } catch (err) {
    console.error(err)
      ...
      await Promise.all(
        events.map(async (e) => {
          const [existingEvent] = await db.query(
            "SELECT * FROM events WHERE coupon_id = ? AND store_id = ? AND event_name = ?",
            [result.insertId, storeId, e]
          )

          if (existingEvent.length === 0) {
            await db.query(
              "INSERT INTO events (coupon_id, store_id, event_name) VALUES (?, ?, ?)",
              [result.insertId, storeId, e]
            )
          }
        })
...
exports.decreaseCouponsOffers = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Coupons and Offers counts updated successfully" })
  } catch (error) {
...
//
exports.updateUserCount = catchAsyncErrors(async (req, res, next) => {
  const { coupon_id, like, dislike, usercount } = req.body
  try {
    // Check if the store exists
    ...
    // Calculate total likes, dislikes, and user counts
    const totallike = storeResult[0].like + (like ? 1 : 0)
    const totaldislike = storeResult[0].dislike + (dislike ? 1 : 0)
    const totalusercount = storeResult[0].user_count + (usercount ? 1 : 0)

    // Update the Coupon
    await db.query(
      `UPDATE coupons SET \`like\` = ${totallike}, dislike = ${totaldislike}, user_count = ${totalusercount} WHERE coupon_id = ${coupon_id}`
    )
  ...
  try {
    ...
    if (req.body.events) {
      ...
      const [currentEvents] = await db.query(
        "SELECT * FROM events WHERE coupon_id = ?",
        [cId]
      )

      // Identify events to be removed
      const eventsToRemove = currentEvents
        .map((event) => event.event_name)
        .filter((event) => !req.body.events.includes(event))

      // Identify events to be added
      const eventsToAdd = req.body.events.filter(
        (event) => !currentEvents.map((e) => e.event_name).includes(event)
      )
      ...
      await Promise.all(
        eventsToRemove.map(async (event) => {
          await db.query(
            "DELETE FROM events WHERE coupon_id = ? AND event_name = ?",
            [cId, event]
          )
        })
      )
      ...
      await Promise.all(
        eventsToAdd.map(async (event) => {
          ...
          const [existingEvent] = await db.query(
            "SELECT * FROM events WHERE coupon_id = ? AND store_id = ? AND event_name = ?",
            [cId, couponResult[0].store_id, event]
          )
    ...
    res
      .status(200)
      .json({ message: `Coupon with ID ${cId} updated successfully` })
  } catch (err) {
    console.error(err)
...
exports.getEventsForCoupons = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    return res.status(200).json({
      message: `Coupon with ID ${cId} fetched successfully`,
...
exports.getDataForEvent = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    return res.status(200).json({
      message: `Event coupons fetched successfully`,
      coupons: couponResult,
    })
  } catch (err) {
    console.error(err)
...
exports.getSingleCoupon = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    // Fetch the single coupon for the store based on cId
    const [couponResult] = await db.query(
      "SELECT coupons.*, store.name, store.logo_url, store.description,store.moreAbout, store.hint, store.faq, store.total_ratings, store.ratings_count, store.coupons FROM coupons INNER JOIN store ON coupons.store_id = store.id  WHERE coupons.store_id = ? AND coupons.coupon_id = ?",
      [storeId, cId]
    )
    ...
    if (couponResult.length === 0) {
      return next(
        new ErrorHandler(
          `Coupon with ID ${cId} not found for store with ID ${storeId}`,
          404
        )
      )
    }
    ...
    res.status(200).json({
      message: `Coupon with ID ${cId} for store with ID ${storeId} fetched successfully`,
...
exports.getCoupons = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    res.status(200).json({
      message: `Coupons for store with ID ${storeId} fetched successfully`,
...
exports.deleteCoupon = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    await db.query("UPDATE store SET coupons = coupons - 1 WHERE id = ?", [
      storeId,
    ])
    ...
    if (
      couponResult[0].type &&
      (couponResult[0].type.toLowerCase() === "deal" ||
        couponResult[0].type.toLowerCase() === "offer")
    ) {
      await db.query(
        "UPDATE store SET offers = GREATEST(offers - 1, 0) WHERE id = ?",
        [storeId]
      )
    }
    ...
    res
      .status(200)
      .json({ message: `Coupon with ID ${cId} deleted successfully` })
  } catch (err) {
    console.error(err)
...
exports.addCategoryStoreId = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Popular store added for category successfully" })
  } catch (err) {
    console.error(err)
...
exports.addEventStoreId = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "Popular store added for Event successfully" })
  } catch (err) {
    console.error(err)
