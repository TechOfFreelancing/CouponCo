...
exports.addCategory = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    const sql =
      "INSERT INTO category (name, logo_url, About,todays_top) VALUES (?, ?, ?, ?)"
...
exports.updateCategory = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    if (categoryResult.length === 0) {
      return next(
        new ErrorHandler(`Category with ID ${categoryId} not found`, 404)
      )
    }
...
// Add a new event
exports.addEvent = catchAsyncErrors(async (req, res) => {
  const { event_name, about, best_offer, avg_disc } = req.body
  const eventLogoFile = req.files.eventLogoFile[0]
  const eventBannerFile = req.files.eventBannerFile
    ? req.files.eventBannerFile[0]
    : null

  try {
    const eventLogoUrl = eventLogoFile
      ? await uploadAndCreateDocument(eventLogoFile)
      : null
    let eventBannerUrl = null
    if (eventBannerFile) {
      eventBannerUrl = await uploadAndCreateDocument(eventBannerFile)
    }
...
exports.updateEvent = catchAsyncErrors(async (req, res, next) => {
  ...
  const eventLogoFile = req.files.eventLogoFile
    ? req.files.eventLogoFile[0]
    : null
  const eventBannerFile = req.files.eventBannerFile
    ? req.files.eventBannerFile[0]
    : null
...
exports.deleteEvent = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: `Event with ID ${eventId} deleted successfully` })
  } catch (err) {
