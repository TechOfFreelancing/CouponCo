...
exports.registerUser = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    const [resObj] = await db.query("SELECT * FROM users WHERE user_id = ?", [
      rId,
    ])
...
exports.forgotPassPost = catchAsyncErrors(async (req, res, next) => {
  try {
    ...
    res.send(
      "Check your inbox, password reset link has been sent successfully!"
    )
  } catch (error) {
    console.log(error)
    return next(new ErrorHandler("Unable to proceed ", 400))
...
exports.getUserDetailsWithCoupons = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    console.error(err)
    return next(
      new ErrorHandler("Unable to fetch user details with saved coupons", 400)
    )
  }
})
...
exports.addContacts = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    const [result] = await db.query(
      "INSERT INTO contacts (user_id, fullname, email, message,isAccepted) VALUES (?, ?, ?, ?,?)",
      [uId, name, email, message, 1]
    )

    return res
      .status(201)
      .json({ message: "Contact Details Send Successfully", user: uId })
  } catch (err) {
    console.log(err)
    return next(new ErrorHandler("Unable to proceed", 400))
  }
...
exports.addAdvertise = catchAsyncErrors(async (req, res, next) => {
  const { uId } = req.params
  const { name, company, url, email, affiliate, message, affiliate_network } =
    req.body
  ...
  try {
    ...
    const query = `INSERT INTO advertise (${insertColumns.join(
      ", "
    )}) VALUES (${placeholders})`
    ...
    return res
      .status(201)
      .json({ message: "Data Send Successfully", user: uId })
  } catch (err) {
    console.log(err)
    return next(new ErrorHandler("Unable to proceed", 400))
  }
...
exports.advertise = catchAsyncErrors(async (req, res) => {
  try {
    const contacts = `select * from advertise`
    const advertiseData = await db.query(contacts)
    res.status(200).json(advertiseData[0])
  } catch (err) {
    console.error(err)
    return res
      .status(400)
      .json({ message: "Unable to fetch user contact data" })
  }
})
//
exports.blogsData = catchAsyncErrors(async (req, res) => {
  try {
    const contacts = `
            select * from blogs
        `
...
exports.blog = catchAsyncErrors(async (req, res) => {
  ...
  try {
    const blogs = `select * from blogs where slug = ?`
    const data = await db.query(blogs, slug)
